from app import app,getbytes,normdate,normtime,today
...
def census_sheet(ws,d):
    ...
    for row in ws.iter_rows(values_only=True):
        ...
        raw=row[0]
        bed=str(int(raw)) if isinstance(raw,(int,float)) and float(raw).is_integer() else str(raw or '').strip().upper()
    ...
    # A valid daily census must contain the primary G/O bed block, not merely an empty template sheet.
    if len(seen)<20: return None
    occ=sorted((EXPECTED[k] for k,v in seen.items() if v),key=int)
    return {'date':d.isoformat(),'occupied':len(occ),'available':27-len(occ),'occupancy_pct':round(100*len(occ)/27,1),'occupied_beds':occ,'status':'REAL'}

def census_history(days):
    data=getbytes(os.getenv('CENSUS_XLSX_URL'))
    if not data:return {}
    wb=load_workbook(io.BytesIO(data),read_only=True,data_only=True)
    out={}; start=today()-datetime.timedelta(days=days-1)
...
def triage_history(days):
    data=getbytes(os.getenv('TRIAGE_XLSX_URL'))
    if not data:return {}
    wb=load_workbook(io.BytesIO(data),read_only=True,data_only=True); events={}; start=today()-datetime.timedelta(days=days-1)
    for ws in wb.worksheets:
        header=None
        ...
        for n,row in enumerate(ws.iter_rows(min_row=hr+1,values_only=True)):
            ...
            fol=str(row[fi] or '').strip() if fi is not None and fi<len(row) else f'R{n}'
            tm=normtime(row[ti] if ti is not None and ti<len(row) else None)
            events.setdefault(d.isoformat(),{})[fol or f'R{n}']=tm
    out={}
    for ds,items in events.items():
        ...
        for tm in items.values():
            ...
            shifts['Matutino' if 420<=mins<=840 else 'Vespertino' if 841<=mins<=1260 else 'Nocturno']+=1
...
def births_history(days):
    ...
    r=requests.get(f'https://docs.google.com/spreadsheets/d/{sid}/export?format=csv&gid={gid}',timeout=20);r.raise_for_status(); start=today()-datetime.timedelta(days=days-1); out={}
...
def history():
    days=max(1,min(int(request.args.get('days',7)),31)); dates=[(today()-datetime.timedelta(days=i)).isoformat() for i in range(days)]
    errors={}
    ...
    rows=[]
    for ds in dates:
        rows.append({'date':ds,'floor':c.get(ds,{'status':'SIN DATO'}),'triage':t.get(ds,{'status':'SIN DATO'}),'births':b.get(ds,{'status':'SIN DATO'})})
